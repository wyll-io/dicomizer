...
import (
	...
	"github.com/suyashkumar/dicom/tag"
)

// TagsD contains the tags to be replaced (D).
const TagsD = []tag.Tag{
	tag.GraphicAnnotationSequence, // Graphic Annotation Sequence
    tag.PersonIdentificationCodeSequence, // Person Identification Code Sequence
    tag.PersonName, // Person Name
    tag.VerifyingObserverName, // Verifying Observer Name
    tag.VerifyingObserverSequence, // Verifying Observer Sequence
}

// Replaced with empty values (0, '', ...)
const TagsZ = []tag.Tag{
    tag.AccessionNumber, // Accession Number
    tag.ContentCreatorName, // Content Creator's Name
    tag.FillerOrderNumberImagingServiceRequest, // Filler Order Number / Imaging Service Request
    tag.PatientID, // Patient ID
    tag.PatientBirthDate, // Patient's Birth Date
    tag.PatientName, // Patient's Name
    tag.PatientSex, // Patient's Sex
    tag.PlacerOrderNumberImagingServiceRequest, // Placer Order Number / Imaging Service Request
    tag.ReferringPhysicianAddress, // Referring Physician's Name
    tag.StudyDate, // Study Date
    tag.StudyID, // Study ID
    tag.StudyTime, // Study Time
    tag.VerifyingObserverIdentificationCodeSequence, // Verifying Observer Identification Code Sequence
}

// TagsX contains the tags to be deleted (X).
const TagsX = []tag.Tag{
    tag.ACR_NEMA_AcquisitionComments, // Acquisition Comments
    tag.AcquisitionContextSequence, // Acquisition Context Sequence
    tag.AcquisitionProtocolDescription, // Acquisition Protocol Description
    tag.ActualHumanPerformersSequence, // Actual Human Performers Sequence
    tag.AdditionalPatientHistory, // Additional Patient's History
    tag.AdmissionID, // Admission ID
    tag.AdmittingDate, // Admitting Date
    tag.AdmittingDiagnosesCodeSequence, // Admitting Diagnoses Code Sequence
    tag.AdmittingDiagnosesDescription, // Admitting Diagnoses Description
    tag.AdmittingTime, // Admitting Time
...
const TagsXZUStar = []tag.Tag{
    (0x0008, 0x1140), // Referenced Image Sequence
    (0x0008, 0x2112), // Source Image Sequence
}

const Tags = append([], TagsD, TagsZ, TagsX, TagsU, TagsZD, TagsXZ, TagsXD, TagsXZD, TagsXZUStar)